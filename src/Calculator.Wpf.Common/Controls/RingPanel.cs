...
namespace Calculator.Wpf.Common.Controls
{
    public class RingPanel : Panel
    {
        ...
        public static readonly DependencyProperty FirstItemAngleProperty = DependencyProperty.Register("FirstItemAngle", typeof(double), typeof(RingPanel),
                                                                                                       new FrameworkPropertyMetadata(0.0, MetadataOptions));

        public double FirstItemAngle
        {
            get { return (double) GetValue(FirstItemAngleProperty); }
            set { SetValue(FirstItemAngleProperty, value); }
        }
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            double maxChildDesiredSide = InternalChildren.OfType<UIElement>()
                                                         .Max(x => Math.Max(x.DesiredSize.Height, x.DesiredSize.Width));
            ...
            for (int i = 0; i < InternalChildren.Count; i++)
            {
                ...
                double itemAngle = FirstItemAngle + anglePerItem * i - Math.PI / 2;
